...
public class UserActivityRankServiceImpl implements UserActivityRankService {
    ...
    public List<RankItemDTO> queryRankList(ActivityRankTimeEnum time, int size) {
        ...
        Map<Long, Integer> userScoreMap = rankList.stream().collect(Collectors.toMap(s-> Convert.toLong(s), s -> Convert.toInt(stringRedisTemplate.opsForZSet().score("pai_" + rankKey,s))));
